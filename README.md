...
| 站点样式 | 手写 CSS embed 进二进制，`GET /site.css`；页脚一键切换亮/暗，记住选择；未选则跟随系统 |
...
│   │   ├── assets/theme.js      # 亮/暗切换，GET /theme.js，localStorage 记住选择
...
| POST | `/api/posts/preview` | 编辑器 Preview（仅 Author；返回消毒 HTML） |
| GET | `/author/login` | Author 登录页 HTML 壳 |
| GET | `/author/posts` | Author 列表壳（数据走 JSON） |
| GET | `/author/new` | 新建 Draft 编辑器壳 |
| GET | `/author/posts/:slug` | 编辑器壳；Slug 只读 |
| GET | `/author/app.js` | Author 区脚本 |
