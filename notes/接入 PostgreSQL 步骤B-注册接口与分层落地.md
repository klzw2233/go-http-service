...
代码合并推送后，GitHub CI 挂在 `TestMigrate_ReleasesLock`——
而**完全相同的命令在本地是通过的**。

根因是一个我一直没注意的事实：

> **`go test ./...` 会并行运行不同包的测试二进制**，
> 并行度由 `-p` 控制，默认等于 `GOMAXPROCS`。

也就是说：

```
internal/db 的测试          ┐
cmd/server 的端到端测试      ├─ 同时运行，共用同一个数据库
internal/repository 的测试   ┘
```

而端到端测试会**启动真实服务**，真实服务启动时**执行迁移**，
执行迁移就要拿**同一把 advisory lock**。

我那条断言写的是：

```go
// 此刻锁必须是空闲的
assert.True(t, acquired, "迁移结束后 advisory lock 必须已释放")
```

但在共用数据库的场景下，另一个进程**正合法地持有它**是完全正常的。
本地通过只是撞运气没碰上。

**这条测试真正要证明的是「锁不会被永久泄露」，而不是「此刻空闲」。** 改法：

```go
require.Eventually(t, func() bool {
    conn.QueryRow(ctx, "SELECT pg_try_advisory_lock($1)", migrationLockID).Scan(&acquired)
    return acquired
}, 20*time.Second, 100*time.Millisecond, "...")
```

释放了的锁在持有者结束后立刻可得；泄露了的**永远**不可得。

### 同一根因还有第二处，而且更安静

排查时顺带发现：`internal/repository` 的测试直接 `INSERT INTO users`，
**但它自己从不执行迁移**——表是靠 `internal/db` 或 `cmd/server` 先建好的。

包并行执行**没有任何顺序保证**。CI 那次通过纯属调度运气，
换个机器、换个核数就可能全挂。

修法是让 `testPool` 自己调 `db.Migrate`。因为迁移是幂等的、
又有 advisory lock 串行化，**多个包同时调它是安全的**——
这恰好是前面那把锁的另一个用途。
...
到这一步为止，本轮一共遇到**三个「侥幸通过」的测试问题**：

| 问题 | 侥幸了多久 | 靠什么暴露 |
|------|-----------|-----------|
| `os/exec` 管道竞态 | 从步骤 A 到步骤 B | 多写了一行关闭日志 |
| advisory lock 断言 | 本地一直通过 | CI 的机器更慢 |
| repository 依赖建表顺序 | CI 那次也通过了 | 主动排查发现 |

共同点很清楚：

1. **全都是测试基础设施的并发假设**，不是业务代码的 bug
2. **全都要等环境变化才暴露**——更多日志、不同核数、不同调度顺序
3. **全都是「时序刚好对上」而非「逻辑正确」**

这解释了 CI 的一个具体价值：它的机器更慢、核数不同、数据库是全新的，
**所以能撞出本地撞不出的时序**。「本地全绿」和「正确」之间的距离，
往往就是这类假设。

> 写测试时值得多问一句：**这条断言依赖的是逻辑，还是时序？**
> 依赖时序的断言（「此刻应该是空闲的」「第一行日志应该是 X」）
> 迟早会在别人的机器上挂掉。
